...
---`;
        ...
        if (!shouldUseSegmentedReview(contractCharCount)) {
            ...
            let completedClauseCount = clauses.length - recalled.length;
            if (completedClauseCount > 0) {
                getIoInstance()?.to(`contract-${contractId}`).emit('clause_progress', {
                    contractId: Number(contractId),
                    reviewed: completedClauseCount,
...
- 不得把措辞偏好、一般最佳实践、模型记忆或“可以更加完善”的内容当作风险。
...
---`;
                    try {
                        const rawClauseResult = await callJsonLLM(clausePrompt, reviewLlmRequestOptions);
                        const clauseResult = normalizeAnalysisResult(
                            enforceKnowledgeGrounding(rawClauseResult, clause.text || '', clauseKnowledge, {
                                allowMissingClauses: false,
                                maxSuggestions: 3,
                            }),
                            clause.text || '',
                        );
